...
class BVProtectIpstore_V647 {
	...
	function __construct($storage_type = BVProtectIpstore_V647::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == BVProtectIpstore_V647::STORAGE_TYPE_FS) {
			$this->storage = new BVProtectIpstoreFS_V647();
		} else {
			$this->storage = new BVProtectIpstoreDB_V647();
		}
	}

	public static function uninstall() {
		BVProtectIpstoreDB_V647::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == BVProtectIpstore_V647::STORAGE_TYPE_DB) {
