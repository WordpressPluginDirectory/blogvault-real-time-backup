...
if (!class_exists('BVProtect_V647')) :
...
class BVProtect_V647 {
	...
	public static function init($mode) {
		...
		if ($mode == BVProtect_V647::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = BVProtectUtils_V647::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(BVProtect_V647::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new BVProtectRequest_V647($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			BVProtectFW_V647::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			BVProtectFW_V647::getInstance($mode, $request, $fw_config, $brand_name)->init();
			BVProtectLP_V647::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		BVProtectIpstore_V647::uninstall();
		BVProtectFW_V647::uninstall();
		BVProtectLP_V647::uninstall();

		BVProtect_V647::removeWPPrepend();
		BVProtect_V647::removePHPPrepend();
		BVProtect_V647::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		BVProtectUtils_V647::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		BVProtect_V647::removeHtaccessPrepend();
		BVProtect_V647::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		BVProtectUtils_V647::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		BVProtectUtils_V647::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		BVProtectUtils_V647::rrmdir($mc_data_dir);
	}
}
endif;