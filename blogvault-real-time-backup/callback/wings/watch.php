	...
	public function process($request) {
		...
		case "getdata":
			...
			if (array_key_exists('lp', $params)) {
				...
				$lp_params = $params['lp'];
				if (!isset($lp_params['bv_check_table']) || $db->isTablePresent($db->getBVTable(BVProtectLP_V585::TABLE_NAME))) {
					$limit = intval($lp_params['limit']);
					$filter = $lp_params['filter'];
					$offset = isset($lp_params['offset']) ? intval($lp_params['offset']) : 0;
					$table = $db->getBVTable(BVProtectLP_V585::TABLE_NAME);
					...
					if ($auto_increment && ($auto_increment < $offset) && $lp_params['enforce_auto_incr_check']) {
						...
						$db->deleteBVTableContent(BVProtectLP_V585::TABLE_NAME, $lp_params['rmfilter']);
			...
			if (array_key_exists('fw', $params)) {
				...
				$fw_params = $params['fw'];
				if (!isset($fw_params['bv_check_table']) || $db->isTablePresent($db->getBVTable(BVProtectFW_V585::TABLE_NAME))) {
					$limit = intval($fw_params['limit']);
					$filter = $fw_params['filter'];
					$offset = isset($fw_params['offset']) ? intval($fw_params['offset']) : 0;
					$table = $db->getBVTable(BVProtectFW_V585::TABLE_NAME);
					$auto_increment = $db->getAutoIncrement($table);
					...
					if ($auto_increment && ($auto_increment < $offset) && $fw_params['enforce_auto_incr_check']) {
						...
						$db->deleteBVTableContent(BVProtectFW_V585::TABLE_NAME, $fw_params['rmfilter']);
